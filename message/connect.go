...
func init() {
	...
	clientIdRegexp = regexp.MustCompile("^[0-9a-zA-Z _\\-|]*$") //wbt增加-|，为了兼容mosquitto的默认cid如："mosqpub|9560-wbtdeMacBo"
...
func (this *ConnectMessage) decodeMessage(src []byte) (int, error) {
	...
	//协议级别 Protocol Level，对于3.1.1版协议，协议级别字段的值是4(0x04)
	...
	//mqtt3.1.1时validClientId可能会不成功，具体参考validClientId里的注释
...
*/
//wbt: 这儿的实现不完全。注意上面所说的关键词“必须”和“可以”。这儿只实现了“必须”没有实现“可以”
//mosquitto的mosquitto_pub没指定clientid时的默认id会包含'-'字符，如"mosqpub|9560-wbtdeMacBo"，
//所以出错导致断开连接。
//所以mosquitto_pub要加上-i指定cid名字，如：
//mosquitto_pub -p 4222  -t im/abcd -m fffs -V mqttv311 -i www
