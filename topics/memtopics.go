...
func (this *memTopics) Subscribe(topic []byte, qos byte, sub interface{}) (byte, error) {
	...
	if qos > MaxQosAllowed { //判断多余，上面ValidQos已经判断了
		qos = MaxQosAllowed
	}

	//创建层级并将sub加入到最后一层nodes的subs中
...
//返回订阅者列表，没有做cache，每次都是直接查询，todo: 要优化可以考虑做cache
...
func (this *snode) sinsert(topic []byte, qos byte, sub interface{}) error {
	...
	if len(topic) == 0 { //没有子层了，那么当前node就是最后一级
		// Let's see if the subscriber is already on the list. If yes, update
		// QoS and then return.
		for i := range this.subs { //遍历订阅者列表，找到同一个sub，更新qos
	...
	//todo: 这儿有个问题，因为完整的topic前面可能都是合法的，但是后面非法。
	//这个sinsert代码导致前面合法的node层级都会被创建，正常情况下是不应该创
	//建这些层级，而是要先验证完整topic是否合法，不合法根本就不允许订阅。
	...
	return n.sinsert(rem, qos, sub) //递归下一层
}
...
func (this *snode) sremove(topic []byte, sub interface{}) error {
	...
	if len(topic) == 0 {
		...
		for i := range this.subs {
			if equal(this.subs[i], sub) { //找到订阅者删除
...
const (
	...
	stateMWC             // Multi-level wildcard 多层通配符
