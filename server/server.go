...
func (s *Server) checkConfiguration() error {
	...
	if s.opts.KeepAlive == 0 {
		s.opts.KeepAlive = DefaultKeepAlive
	}

	if s.opts.ConnectTimeout == 0 {
		s.opts.ConnectTimeout = DefaultConnectTimeout
	}

	if s.opts.AckTimeout == 0 {
		s.opts.AckTimeout = DefaultAckTimeout
	}

	if s.opts.TimeoutRetries == 0 {
		s.opts.TimeoutRetries = DefaultTimeoutRetries
	}

	s.sessMgr, err = sessions.NewManager("mem")
	if err != nil {
		return err
	}

	s.topicsMgr, err = topics.NewManager("mem")
...
func (s *Server) AcceptLoop(clr chan struct{}) {
	...
	// to 0 at the beginning s function. So we need to get the actual port
	if opts.Port == 0 {
	...
	//todo：这儿isRunning加锁我觉得不是很必要，程序都要退出了，还锁不锁干什么用，
	//这儿的锁多处共享会影响性能，比如下面的createClient中就共享锁了
	for s.isRunning() {
		conn, err := l.Accept()
		if err != nil {
...
// Note that s function may be called during config reload, s is why
...
func (s *Server) setInfoHostPortAndGenerateJSON() error {
	// When s function is called, opts.Port is set to the actual listen
...
func (s *Server) StartProfiler() {
	...
	go func() {
		// if s errors out, it's probably because the server is being shutdown
		err := srv.Serve(l)
		if err != nil {
			s.mu.Lock()
			shutdown := s.shutdown
...
func (s *Server) createClient(conn net.Conn) *client {
	...
	// list of connections to close. It won't contain s one, so we need
	...
	// s new client would not push us over the max
	...
	// Check for Auth. We schedule s timer after the TLS handshake to avoid
	// the race where the timer fires during the handshake and causes the
	// server to write bad data to the socket. See issue #432.
	if authRequired { //客户端连接上来以后要马上发第一条connect消息，如果默认超时2秒就关闭连接，防止用户连接以后不发消息
...
func (s *Server) updateServerINFOAndSendINFOToClients(urls []string, add bool) {
	...
	if wasUpdated {
		...
		// Add s server client connect ULRs first...
...
// "any" interfaces, s call returns the list of resolved IP addresses.
