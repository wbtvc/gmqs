...
type Sublist struct {
	sync.RWMutex
	genid     uint64 //操作自增id,insert或者remove都会自增
	matches   uint64
	cacheHits uint64
	inserts   uint64
	removes   uint64
...
func (s *Sublist) addToCache(subject string, sub *subscription) {
	for k, r := range s.cache {
		if matchLiteral(k, subject) { //如果匹配
			// Copy since others may have a reference.
			nr := copyResult(r) //先复制旧队列到nr，然后下面把新的sub加入到nr中
			if sub.queue == nil {
...
func (s *Sublist) removeFromCache(subject string, sub *subscription) {
	for k := range s.cache {
		if !matchLiteral(k, subject) { //cache查找是否匹配，找到则从cache中删除
			continue
		}
...
func (s *Sublist) Match(subject string) *SublistResult {
	s.RLock()
	atomic.AddUint64(&s.matches, 1) //查找次数统计
	rc, ok := s.cache[subject]      //先在cache中查找
	s.RUnlock()
	if ok {
		atomic.AddUint64(&s.cacheHits, 1) //cache命中统计
		return rc
	}
	...
	s.Lock()
	matchLevel(s.root, tokens, result) //真正查找

	// Add to our cache
	s.cache[subject] = result //cache缓存
	// Bound the number of entries to sublistMaxCache
	if len(s.cache) > slCacheMax { //缓存超过1024个
		for k := range s.cache { //删除最早的一个缓存
			delete(s.cache, k)
			break
		}
	}
...
func shrinkAsNeeded(sl []*subscription) []*subscription {
	...
	if csl <= 8 { //切片容量不大时不用缩小
		return sl
	}
	pFree := float32(csl-lsl) / float32(csl)
	if pFree > 0.50 { //当剩余的空间超过一半时缩小切片容量
...
func (s *Sublist) Stats() *SublistStats {
	...
	tot, max := 0, 0
	for _, r := range s.cache { //统计缓存的订阅数量和最长的一个订阅队列长度
...
func (s *Sublist) numLevels() int { //返回最深层数
	...
	for i := 0; i < ls; i++ {
		...
		case pwc: //*
			...
			if i == 0 || subject[i-1] == btsep {
				...
				} else if subject[i+1] == btsep { //下一个字符是分隔符
